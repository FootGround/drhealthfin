...
// ============================================================================
// RADAR CHART COMPONENT — Spider chart for 6 pillars
// ============================================================================

const RadarChart = ({ pillars, c }: {
  pillars: Pillars;
  ...
  return (
    ...
    >
      ...
      {/* Axis lines from center to each pillar */}
      {items.map((_, i) => {
        const end = pt(maxR, i);
      ...
      >
        ...
        <main style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '20px', animation: 'fadeUp 0.5s ease-out' }}>
          ...
          <div style={{ width: '100%', marginTop: spacing.lg }}>
            <RadarChart pillars={pillars} c={c} />
          </div>

          <div style={{ fontSize: '12px', color: c.muted, marginTop: spacing.sm, opacity: 0.6 }}>{data.updatedAt}</div>
        </main>
